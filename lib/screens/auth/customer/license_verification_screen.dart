        ...
        try {
          ...
          if (mounted) {
            setState(() {
              _user = userModel;
              _loadingUser = false;
              if (_licenseNumberController.text.isEmpty && userModel.licenseNumber != null) {
  ...
  Future<void> _submitVerification() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Driving License uploaded! Status is now pending verification.'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Verification upload failed: $e'), backgroundColor: Colors.redAccent),
        );
      }
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(color: isDark ? Colors.white70 : AppColors.secondaryBlue, fontWeight: FontWeight.w500),
              ),
            ],
          ),
        ),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(Icons.logout_rounded, color: isDark ? Colors.white70 : AppColors.secondaryBlue),
            onPressed: _logout,
            tooltip: 'Logout',
          ),
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 16),
              ListTile(
                leading: const Icon(Icons.badge_outlined, color: AppColors.primaryOrange),
                title: const Text('License Number', style: TextStyle(fontSize: 11, color: Colors.grey)),
                subtitle: Text(_user?.licenseNumber ?? 'N/A', style: const TextStyle(fontWeight: FontWeight.bold)),
              ),
              ListTile(
                leading: const Icon(Icons.class_outlined, color: AppColors.primaryOrange),
                title: const Text('Class Type', style: TextStyle(fontSize: 11, color: Colors.grey)),
                subtitle: Text(_user?.licenseClass ?? 'N/A', style: const TextStyle(fontWeight: FontWeight.bold)),
              ),
              ListTile(
                leading: const Icon(Icons.date_range_outlined, color: AppColors.primaryOrange),
                title: const Text('Expiration Date', style: TextStyle(fontSize: 11, color: Colors.grey)),
                subtitle: Text(_user?.licenseExpiry ?? 'N/A', style: const TextStyle(fontWeight: FontWeight.bold)),
              ),
            ],
          ),
        ),
        ...
        ElevatedButton.icon(
          ...
          label: const Text('Check Status Now', style: TextStyle(fontWeight: FontWeight.bold)),
        ),
      ],
    );
  }

  Widget _buildUploadForm(String companyName, bool isDark, {bool isRejection = false}) {
    return Form(
      key: _formKey,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          ...
          const SizedBox(height: 24),
          if (isRejection && _user?.licenseRejectionReason.isNotEmpty == true) ...[
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(color: isDark ? Colors.white70 : Colors.red[800], fontSize: 13, height: 1.4),
                  ),
                ],
              ),
            ),
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('  Driving License Number', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                ...
                const Text('  License Classification Class', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                const SizedBox(height: 6),
                DropdownButtonFormField<String>(
                  initialValue: _selectedClass.contains('DA') ? 'Class DA (Automatic Car)' : 'Class D (Car)',
                  decoration: InputDecoration(
                    prefixIcon: const Icon(Icons.class_outlined, color: AppColors.primaryOrange),
                  ...
                  items: _licenseClasses.map((String cls) {
                    return DropdownMenuItem<String>(
                      value: cls,
                      child: Text(cls, style: TextStyle(color: isDark ? Colors.white : Colors.black, fontSize: 13)),
                    );
                  }).toList(),
                  onChanged: (val) {
                    if (val != null) {
                      setState(() {
                        _selectedClass = '${val.split(' ')[0]} ${val.split(' ')[1]}';
                      });
                    }
                  },
                ),
                ...
                // Expiry Date picker
                const Text('  License Expiry Date', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                const SizedBox(height: 6),
                InkWell(
                ...
                // Card Photo Upload
                const Text('  Driving License Card Photo', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                InkWell(
                  onTap: _pickImage,
                  child: Container(
                    ...
                    child: _licenseImage == null
                        ? Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              const Icon(Icons.add_photo_alternate_outlined, size: 40, color: AppColors.primaryOrange),
                              const SizedBox(height: 8),
                              const Text('Upload photo card from gallery', style: TextStyle(fontSize: 12, color: Colors.grey)),
                            ],
                          )
                        : const Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              Icon(Icons.check_circle_outline, size: 44, color: Colors.green),
                              SizedBox(height: 8),
                              Text(
                                'Card Photo Selected',
                              ...
                              SizedBox(height: 4),
                              Text('Tap to change card photograph', style: TextStyle(fontSize: 11, color: Colors.grey)),
                            ],
                          ),
                  ),
                ),
