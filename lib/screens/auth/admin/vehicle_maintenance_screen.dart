...
class _VehicleMaintenanceViewState extends State<VehicleMaintenanceView> {
  ...
  void _showAddEditJobDialog({MaintenanceJobModel? job}) {
    ...
    final costController = TextEditingController(
      text: job != null ? job.cost.toString() : '',
    );
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SwitchListTile(
                        ...
                        onChanged: (val) =>
                            setDialogState(() => showToCustomer = val),
                      ),
                    ],
                  ),
                ),
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    if (isEdit) {
                      ...
                      await _maintenanceService.addMaintenanceJob(
                        MaintenanceJobModel(
                          id: '',
                          vehicleId: selectedVehicle.id,
                          vehicleName:
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading) {
      return const Center(
        child: LoadingWidget(message: 'Syncing maintenance log...'),
      );
    }
    ...
    final filteredJobs = _jobs.where((job) {
      final matchesStatus =
          _selectedFilter == 'All' ||
          job.status.toLowerCase() == _selectedFilter.toLowerCase();
      final matchesVehicle =
          _selectedVehicleFilter == 'All' ||
          job.vehicleId == _selectedVehicleFilter;
      final matchesSearch =
          job.vehicleName.toLowerCase().contains(_searchQuery) ||
      ...
      child: Column(
        ...
        children: [
          ...
          GridView.count(
            ...
            mainAxisSpacing: 16,
            shrinkWrap: true,
            childAspectRatio: isDesktop ? 2.2 : 1.32,
        ...
        rows: jobs.map((job) {
          ...
          return DataRow(
            cells: [
              ...
              DataCell(
                Text(
                  'RM ${job.cost.toStringAsFixed(2)}',
                  style: TextStyle(color: textPrimary),
                ),
              ),
              DataCell(
                Text(job.startDate, style: TextStyle(color: textSecondary)),
              ),
              DataCell(
                Text(job.endDate, style: TextStyle(color: textSecondary)),
              ),
              DataCell(
                Container(
                  padding: const EdgeInsets.symmetric(
              ...
              DataCell(
                Text(
                  job.description.isNotEmpty ? job.description : 'N/A',
                  style: TextStyle(color: textSecondary),
                  overflow: TextOverflow.ellipsis,
                ),
              ),
                ...
                Row(
                  ...
                  children: [
                    DropdownButton<String>(
                      ...
                      onChanged: (val) {
                        if (val != null) _updateStatus(job.id, val);
                      },
                    ),
                    IconButton(
                      icon: const Icon(
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'Cost: RM ${job.cost.toStringAsFixed(2)}',
                style: const TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.bold,
                  color: AppColors.primaryOrange,
